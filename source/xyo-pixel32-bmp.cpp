	...
	namespace Pixel32 {
		...
		void Bmp::setPixelPal8(uint32_t x, uint32_t y, BmpRgbQuad &c) {
			...
			for(index = 0; index < 256; index++) {
				if((c.rgbBlue == palette[index].rgbBlue)
		...
		bool Bmp::getPaletteColor(uint8_t &index, BmpRgbQuad &c) {
			...
			for(index2 = 0; index2 < index2p; index2++) {
				if((c.rgbBlue == palette[index2].rgbBlue)
					&& (c.rgbRed == palette[index2].rgbRed)
					&& (c.rgbGreen == palette[index2].rgbGreen)
					&& (c.rgbReserved == palette[index2].rgbReserved)) {
					break;
				}
			};
